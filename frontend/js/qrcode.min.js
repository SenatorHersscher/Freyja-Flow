/**
 * QRCode.js micro standalone canvas generator
 * Lightweight zero-dependency QR code renderer for Freyja Flow
 */
(function(){
...
setupTypeInfo:function(test,maskPattern){var data=(1<<3)|maskPattern;var bits=QRUtil.getBCHTypeInfo(data);for(var i=0;i<15;i++){var mod=(!test&&((bits>>i)&1)==1);if(i<6){this.modules[i][8]=mod;}else if(i<8){this.modules[i+1][8]=mod;}else{this.modules[this.moduleCount-15+i][8]=mod;}}for(var i=0;i<15;i++){var mod=(!test&&((bits>>i)&1)==1);if(i<8){this.modules[8][this.moduleCount-i-1]=mod;}else if(i<9){this.modules[8][15-i-1+1]=mod;}else{this.modules[8][15-i-1]=mod;}}this.modules[this.moduleCount-8][8]=(!test);},
...
QRRSBlock.getRsBlockTable=function(typeNumber,errorCorrectLevel){switch(typeNumber){case 1:return[1,26,19];case 2:return[1,44,34];case 3:return[1,70,55];case 4:return[1,100,80];case 5:return[1,134,108];default:return[2,86,68];}};
